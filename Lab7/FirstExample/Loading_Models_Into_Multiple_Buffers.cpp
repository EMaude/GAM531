...
GLuint texture[100];	//Buffers required for loading Textures
...
void init(void)
{
	...
	GLfloat colorData[NumVertices + Sphere_NumVertices][4];

	for (int i = 0; i < NumVertices + Sphere_NumVertices; i++)
	{
		colorData[i][0] = randomFloat(0, 1);
		colorData[i][1] = randomFloat(0, 1);
		colorData[i][2] = randomFloat(0, 1);
		colorData[i][3] = 1.0f;
	}
...
//To draw multiple objects using multiple buffers

//Here is the algorithm of rendering many models into many buffers (one model per buffer): 

/*
1 - Create buffers using glGenBuffer
glGenBuffers(2, Buffers);

2 - Bind each buffer as an array buffer.
glBindBuffer(GL_ARRAY_BUFFER, Buffers[0]);

3 - Fill each buffer with vertex data.
glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);

4 - Create and enable a vertex attrib array in the buffer and enable it
glEnableVertexAttribArray(0);

5 - Bind the buffer with the variable in the shader
glBindAttribLocation(program, 0, "vPosition");

Repeat steps 2 to 5 for until all your models have been loaded into buffers.

Here is the code to load two models:
...
And all your models have been loaded :-)

When it comes to drawing:

Bind (activate) the buffer that you want to draw:
glBindBuffer(GL_ARRAY_BUFFER, Buffers[0]);
...
Repeat the same procedure for other buffers containing other models :-)
